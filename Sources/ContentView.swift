...
struct ContentView: View {
    ...
    var body: some View {
        ...
        view = AnyView(view.onReceive(NotificationCenter.default.publisher(for: .altitudeSeatConfigurationDidChange)) { _ in
            for context in commandPaletteSwitcherWindowContexts() {
                for workspace in context.tabManager.tabs { workspace.refreshAltitudeSeatTitles() }
            }
        })
    ...
    private func commandPaletteSurfaceKindLabel(for panelType: PanelType) -> String {
        ...
        case .altitudeSeatPlaceholder:
            return String(localized: "commandPalette.kind.altitudeSeat", defaultValue: "Seat")
        }
    }
...
struct VerticalTabsSidebar: View, Equatable {
    ...
    private func cmuxSidebarSurfaceKind(for panelType: PanelType) -> CmuxSidebarSurfaceKind {
        ...
        case .extensionBrowser:
            return .unknown
        case .workspaceTodo, .cloudVMLoading, .altitudeSeatPlaceholder:
            return .unknown
        }
    }
